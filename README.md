...
Today: 21 streaming indicators in `tickglide-ta`, verified against talipp, TA-Lib, and pandas-ta, benchmarked with Criterion; the `tickglide-strategy` trait surface for writing signal generators is now published as a `0.0.1` preview alongside. MSRV 1.95.

## Status

| Crate              | Version       | Status        | Purpose                                              |
|--------------------|---------------|---------------|------------------------------------------------------|
| `tickglide-core`     | [![crates.io](https://img.shields.io/crates/v/tickglide-core.svg)](https://crates.io/crates/tickglide-core) | published     | Shared structs, traits, and public interfaces        |
| `tickglide-ta`       | [![crates.io](https://img.shields.io/crates/v/tickglide-ta.svg)](https://crates.io/crates/tickglide-ta) | published     | Streaming technical analysis indicators              |
| `tickglide-strategy` | [![crates.io](https://img.shields.io/crates/v/tickglide-strategy.svg)](https://crates.io/crates/tickglide-strategy) | preview       | User-facing API for authoring strategies executed by the engine |
| `tickglide-engine`   | —             | planned       | Streaming runtime — event loop, multi-TF state, execution |
| `tickglide-sim`      | —             | planned       | Backtester and forward-tester with honest fill models |
| `tickglide-ob`       | —             | planned       | Order book reconstruction and L2/L3 event handling   |

`tickglide-ta` (`0.21.0`) and `tickglide-core` (`0.3.0`) are published on crates.io. Both are feature-complete for v1.0; the v1.0 bump is gated on validation under real streaming workloads.

`tickglide-strategy` (`0.0.1`) ships the `SignalGenerator` trait surface plus a complete `test_util` kit (`FakeMarketSnapshot`, `RecordingMarketSignalConfig`, `EnforcingMarketSnapshot`, `FakeEngine`) for unit-testing generators without a live engine. Released as a **preview**: the engine that consumes the trait is not yet implemented, so the contract has not been validated end-to-end. Pin to an exact `0.0.x` version during the preview period — each bump should be assumed breaking. The `0.1.0` cut is gated on engine consumption.
...
- **Numerical correctness.** NaN, Inf, and warmup semantics are documented per-indicator, not glossed over. `tickglide-ta` is verified against talipp, TA-Lib, and pandas-ta.
...
Design notes and per-indicator writeups at [luksza.org/category/tickglide](https://luksza.org/category/tickglide/):
...
- [tickglide-ta: Real-Time Technical Analysis for Rust](https://luksza.org/2026/tickglide-ta-real-time-technical-analysis-for-rust/) — 2026-03-08
...
1. `tickglide-core` v1.0 - currently at `0.3.0`; v1.0 cut once validated under real streaming workloads
2. `tickglide-ta` v1.0 - currently at `0.21.0`; v1.0 cut on the same gate
3. `tickglide-strategy` v0.1.0 - currently at `0.0.1` preview; `0.1.0` cut once the engine has consumed the trait end-to-end
4. `tickglide-engine` - streaming runtime with multi-timeframe state, deterministic event loop, and wasmtime-based generator registration
5. `tickglide-sim` - backtester with realistic fills, slippage, and drawdown reporting
6. `tickglide-ob` - streaming order book with deterministic event replay
